            ...
            <div class="col-sm-12 col-md-8 col-lg-6 ps-xl-75 ps-0">
                <div
                    ...
                    <div class="dt-buttons btn-group flex-wrap">
                        <a href="{{ route('articals.create') }}" class="btn add-new btn-primary mt-50">
                            <span>{{ __('Add New Artical') }}</span>
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <tr>
                        <td>
                            @if ($artical->image)
                                <span class="avatar"><img class="round"
                                        src="{{ env('APP_URL') . 'content/' . $artical->image }}" alt="avatar"
                                        height="40" width="40">
                                </span>
                            @else
                                {{ __('No image') }}
                        ...
                        <td>
                            ...
                            if (strlen($description) > $limit) {
                                echo substr($description, 0, $limit) . '<a href="' . route('articals.edit', Crypt::encrypt($artical->id)) . '">... See More</a>';
                            } else {
                                echo $description;
                            }
                            ?>
                        ...
                        <td>
                            {{-- {{ $artical->status }} --}}
                            <span class="{{ $artical->status_class }}">{{ ucfirst($artical->status) }}</span>
                        </td>
                        <td>
