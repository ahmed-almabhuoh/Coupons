            ...
            <div class="col-sm-12 col-md-8 col-lg-6 ps-xl-75 ps-0">
                <div
                    ...
                    <div class="dt-buttons btn-group flex-wrap">
                        <a href="{{ route('offers.create') }}" class="btn add-new btn-primary mt-50">
                            <span>{{ __('Add New Offer') }}</span>
                        </a>
                    </div>
                </div>
            </div>
                ...
                @foreach ($offers as $offer)
                    <tr>
                        <td>
                            @if ($offer->image)
                                <span class="avatar"><img class="round"
                                        src="{{ env('APP_URL') . 'content/' . $offer->image }}" alt="avatar"
                                        height="40" width="40">
                                </span>
                            @else
                                {{ __('No image') }}
                        ...
                        <td>
                            @if ($offer->country)
                                <a
                                    href="{{ route('countries.edit', Crypt::encrypt($offer->country->id)) }}">{{ $offer->country->name }}</a>
                            @else
                                {{ __('No Country') }}
                            @endif
                        </td>
                        <td>
                            ...
                            <a href="{{ $offer->btn_action }}" target="_blank"
                                class="btn btn-relief-info">{{__('Go !!')}}</a>
                            {{-- @endif --}}
                        </td>
                        @if (auth()->user()->can('delete-offer') ||
                                auth()->user()->can('edit-offer'))
                            <td>
                                ...
                                @if (auth()->user()->can('edit-offer'))
                                    <a href="{{ route('offers.edit', Crypt::encrypt($offer->id)) }}"
                                ...
                                @if (auth()->user()->can('delete-offer'))
                                    <button type="button"
                                        onclick="confirmationDelete('{{ Crypt::encrypt($offer->id) }}', this, '{{ auth('admin')->user()->lang }}')"
